...
Splash.attributes.add("posX", {
	type: "curve"
}), Splash.attributes.add("posY", {
	type: "curve"
}), Splash.attributes.add("posZ", {
	type: "curve"
}), Splash.attributes.add("Time", {
	type: "number"
...
Anim2.attributes.add("fwddir", {
	type: "boolean",
	"default": !0
}), Anim2.attributes.add("offsetrotY", {
	type: "number"
}), Anim2.attributes.add("minrotY", {
	type: "number"
}), Anim2.attributes.add("aimrotY", {
	type: "number"
}), Anim2.attributes.add("radius1", {
	type: "curve"
}), Anim2.attributes.add("height1", {
	type: "curve"
}), Anim2.prototype.initialize = function() {
	...
	var i = this.r1 * Math.sin(t * pc.math.DEG_TO_RAD),
		s = this.r1 * Math.cos(t * pc.math.DEG_TO_RAD);
	this.entity.setPosition(i, this.h1, s), this.entity.setEulerAngles(90, t, 0)
}, Anim2.prototype.update = function(t) {}, Anim2.prototype.updateAnim = function(t) {
	if(-t <= this.minrotY) {
	...
	var r = this.r1 * Math.sin(s),
		h = this.r1 * Math.cos(s);
...
ShowControl.attributes.add("rot1", {
	type: "number"
}), ShowControl.attributes.add("rot2", {
	type: "number"
}), ShowControl.attributes.add("rot3", {
	type: "number"
}), ShowControl.attributes.add("rot4", {
	type: "number"
}), ShowControl.attributes.add("show1", {
	type: "entity"
}), ShowControl.attributes.add("show2", {
	type: "entity"
}), ShowControl.attributes.add("offset", {
	type: "number"
}), ShowControl.prototype.initialize = function() {
...
}, ShowControl.prototype.update = function(t) {}, ShowControl.prototype.updateScene = function(t) {
	t *= -1;
	var o, s;
...
ScaleAnim.attributes.add("startangle", {
	type: "number"
}), ScaleAnim.attributes.add("speed", {
	type: "number",
	"default": 2
	...
	if(!this.scaleanim)
		if(this.startanim)
			if(this.timer += this.speed * t, this.timer > this.scaletime) this.entity.setLocalScale(this.InitScale.x, this.InitScale.y, this.InitScale.z), this.scaleanim = !0;
			else {
...
LightAnim.attributes.add("angle", {
	type: "curve"
}), LightAnim.attributes.add("speed", {
	type: "number"
}), LightAnim.prototype.initialize = function() {
	var t = this.app;
...
TextureAnim.attributes.add("material", {
	type: "asset"
}), TextureAnim.attributes.add("texture", {
	type: "asset"
}), TextureAnim.attributes.add("speed", {
	type: "number"
	...
	if(this.cameracontrol.cantopanim && (this.timer += this.speed * t, this.timer > 1)) {
		...
		var e = this.index % 4,
			i = Math.floor(this.index / 4);
		this.mat.emissiveMapOffset = new pc.Vec2(.25 * e, .5 * i), this.mat.update()
	}
};
var Animfloat = pc.createScript("animfloat");
Animfloat.attributes.add("aa", {
	type: "number"
}), Animfloat.attributes.add("ww", {
	type: "number"
}), Animfloat.attributes.add("gg", {
	type: "number"
}), Animfloat.prototype.initialize = function() {
	this.t = 0;
...
CameraControl.prototype.initialize = function() {
	...
	var n = document.getElementById("menubtn"),
		r = document.getElementById("return1btn"),
		a = document.getElementById("return2btn"),
		m = document.getElementById("downbtn");
	this.downbtn = m;
	...
	this.Control(t)
}, CameraControl.prototype.swap = function(t) {}, CameraControl.prototype.dolly = function() {
	var t = 1; - this.rotY <= 108 && (t = -this.rotY / 108), this.targetZoom = (1 - t) * this.minzoom + t * this.maxzoom, this.targetZoom = pc.math.clamp(this.targetZoom, this.minzoom, this.maxzoom), this.rotY < this.minrotY + this.toprotY && this.rotY >= this.minrotY - 50 ? (this.cantopanim = !0, t = (-this.minrotY + this.rotY) / this.toprotY, t = pc.math.clamp(t, 0, 1), this.targetZoom = this.maxzoom * t + this.topzoom * (1 - t), this.targetaddheight = this.topaddheight * (1 - t)) : (this.targetaddheight = 0, this.cantopanim = !1)
...
OffsetAnim.attributes.add("offset", {
	type: "curve"
}), OffsetAnim.attributes.add("animType", {
	type: "number",
	"default": 0,
	"enum": [{
		OffsetX: 0
...
}), OffsetAnim.attributes.add("speed", {
	type: "number"
}), OffsetAnim.prototype.initialize = function() {
	var t = this.app;
	...
	if(this.timer += this.speed * t, this.timer > 1) this.entity.script.enabled = !1;
	else {
...
ResourcesManager.prototype.initialize = function() {
	...
	var s, t, r, o, n, d, i, f, g, l, y, p, c, B, u, h = a.root.findByName("model"),
		m = a.root.findByName("zhuti"),
		v = a.root.findByName("part1"),
		b = a.root.findByName("part2"),
		N = a.root.findByName("part3"),
		P = a.root.findByName("part4"),
		T = a.root.findByName("part5"),
		M = a.root.findByName("part6"),
		R = a.root.findByName("part7"),
		z = a.root.findByName("part8"),
		w = a.root.findByName("part9"),
		S = a.root.findByName("part10"),
		j = a.root.findByName("part11"),
		k = a.root.findByName("part12"),
		q = a.assets.findByTag("item");
	t = q.length;
...
}, ResourcesManager.prototype.update = function(a) {}, ResourcesManager.prototype.setProgress = function(a) {
	1 === a && (this.camera1.script.enabled = !0)
};
var BoundBox = pc.createScript("BoundBox");
BoundBox.attributes.add("size", {
	type: "vec3"
}), BoundBox.attributes.add("offset", {
	type: "vec3"
}), BoundBox.attributes.add("name", {
	type: "string"
}), BoundBox.prototype.initialize = function() {
	this.collider = new pc.BoundingBox(this.entity.getPosition(), this.size), this.collider.center.add(this.offset)
}, BoundBox.prototype.update = function(t) {}, BoundBox.prototype.setBoundBox = function() {
...
}, SetViewControl.prototype.update = function(t) {}, SetViewControl.prototype.doRaycast = function(t, o) {
...
TipAnim.attributes.add("offset", {
	type: "curve"
}), TipAnim.attributes.add("animType", {
	type: "number",
	"default": 0,
	"enum": [{
		OffsetX: 0
...
}), TipAnim.attributes.add("speed", {
	type: "number"
}), TipAnim.prototype.initialize = function() {
	this.app;
