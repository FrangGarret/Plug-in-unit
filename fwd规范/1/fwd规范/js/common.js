...
function fwd_alert(message, callback) {
	...
	okBubbton.className = 'iconfont icon-quxiao';
...
//-------------------------------------------------------------------------------------------------------------------------------
  ...
  $(document).on('click','.side_ul-li_public',function(){
    ...
    if( $(this).hasClass('side_ul-li') ){
      ...
      createJson('target_li_inedx',$(this).index(),jsons);

    }else{
      createJson('target_li_parentNode_index',$(this).attr('ft'),jsons);
    }
  ...
  window.onpopstate = function(ev) {
    ...
    if( obj ){
      ...
      console.log(obj)
      var target_li_index=ev.state.target_li || ''
      var target_li_parentNode_index = ev.state.target_li_parentNode_index || ''
      console.log(prev_page)
      if(target_li_index==''){
        $('.side_ul-li').eq(target_li_parentNode_index).addClass('active').siblings().removeClass('active')
        $('.main-content').fadeOut(0).fadeIn(1500).load(prev_page+".html")
      }else{
        $('.side_ul-li').eq(target_li_index).addClass('active').siblings().removeClass('active')
        $('.main-content').fadeOut(0).fadeIn(1500).load(prev_page+".html")
      }
    }else{
      $('.side_ul-li').eq(0).addClass('active').siblings().removeClass('active')
